...
async function playGuessGame() {
    ...
    do {
        var guessedNumInput = await inquirer.prompt([
            {
                type: "number",
                name: "guessedNum",
                message: "Please enter the number of your choice to guess from 0 to " +
                    guessDifficulty +
                    " :",
            },
        ]);
        if (guessedNumInput.guessedNum === randomNumber) {
            console.clear();
            let rainbowTitle = chalkAnimation.rainbow("Congratulations! " + playerInput.playerName + " Wins!");
            await sleep();
            rainbowTitle.stop();
            console.log(chalk.greenBright("You guessed in " + numberOfAttempts + " attempt."));
            isGuessNum = true;
        }
        else if (guessedNumInput.guessedNum > guessDifficulty) {
            numberOfAttempts++;
            console.log(chalk.redBright("Your guessed number is out of range. Number of attempt: " +
                numberOfAttempts));
        }
        else if (guessedNumInput.guessedNum > randomNumber) {
            numberOfAttempts++;
            console.log(chalk.yellowBright("Lower the number please. Number of attempt: " + numberOfAttempts));
        }
        else if (guessedNumInput.guessedNum < randomNumber) {
            numberOfAttempts++;
            console.log(chalk.yellowBright("Higher the number please. Number of attempt: " + numberOfAttempts));
        }
        else {
            numberOfAttempts++;
            console.log(chalk.redBright("Something went wrong with Your guessed number. Number of attempt: " +
                numberOfAttempts));
        }
    } while (!isGuessNum);
}
